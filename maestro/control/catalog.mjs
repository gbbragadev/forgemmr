  ...
  const actions = [
    action({
      ...
      fields: [
        ...
        field("target", "Target (opcional)", "select", { options: ["cf-pages", "cf-workers", "vercel", "gh-pages"] }),
  ...
  for (const pipeline of pipelines) {
    ...
    actions.push(
      ...
      action({
        ...
        risk: "guarded",
        enabled: status === "done",
        blockedReason: "A simulação manual fica disponível quando a pipeline termina.",
