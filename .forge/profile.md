...
> `forge profile init`) para iniciar um projeto diferente. O bloco de configuração
> (fenced "forge-config") abaixo são os knobs que a engine lê (JSON, zero-dep); o corpo
> em markdown é o contexto narrativo, injetado nos prompts como fundação do projeto.
